...
class CandidateDetector:
    ...
    NUMERIC_PATTERN = re.compile(
        r'[\$€£₹]\s*\d|'
        r'\d+(?:,\d{3})*(?:\.\d+)?\s*(?:million|billion|mn|bn|crore|lakh|%|percent)',
        re.IGNORECASE,
    )
    ...
    TEMPORAL_PATTERN = re.compile(
        r'\b(?:FY|CY|fiscal\s+year|quarter|Q[1-4]|H[12])\s*\d{2,4}\b|'
        r'\b(?:20\d{2}|19\d{2})\b|'
    ...
    ACTION_PATTERN = re.compile(
        ...
        r'approved|ratified|amended|established)\b',
        re.IGNORECASE,
    )

    DEFINITIVE_PATTERN = re.compile(
        r'\b(?:was|were|is|are|has been|have been|will be|became|'
        r'totaled|amounted to|stood at|reached|recorded)\b',
        re.IGNORECASE,
    )
    ...
    def score_block(self, block: ParsedBlock) -> float:
        ...
        score += min(0.1, definitive_matches * 0.05)
